...
public class AdministrationControllerTest {
    ...
    public void removeConnectedUserTest() throws Exception {
        this.mockMvc.perform(post("/users/1/remove").with(csrf()))
                .andDo(print())
    ...
    public void testBlockUser() throws Exception {
        this.mockMvc.perform(post("/users/2/block").with(csrf()))
                .andDo(print())
                .andExpect(authenticated())
                .andExpect(status().isOk())
                .andExpect(content().string(containsString("Заблокирован")));
    }
}
